...
System.out.println("Build Info Industry: " + computer_move.getIndustryID());	
		...
		else if (action_id == BrassActionEnum.DOUBLE_RAIL.getValue())
		{
			int link_id = computer_move.getLinkID();
			buildLink(link_id, computer_player_id);
			int expensive_link_id = computer_move.getExpensiveLinkID();
			if(canBuildExpensiveLink(expensive_link_id,  link_id))
			{
			buildExpensiveLink(link_id, computer_player_id);
			}
		}
