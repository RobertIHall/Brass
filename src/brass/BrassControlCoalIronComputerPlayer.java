...
class BrassControlCoalIronComputerPlayer implements BrassComputerPlayer
{
	...
	public BrassComputerPlayerAction getBrassMove()
	{
		...
		if(!brass_game.getPhase())
		{
			...
			{
				firstTurn(computer_move, computer_player_id, BrassIndustryEnum.COTTON.getValue());
				turnCount++;
			...
			if(stopFirstSell && !shipBuilt)
			{
				...
				computer_move.selectTechUpgradeAction(getCardForNonBuildAction(computer_player_id), BrassIndustryEnum.SHIP.getValue(), BrassIndustryEnum.SHIP.getValue());
				shipBuilt = true;
				stopFirstSell = false;
			...
			if(firstLoanTaken && !actualShipNotBuilt)
			{
				...
				else
				{
					...
					if(coalTech == 1)
					{
						computer_move.selectTechUpgradeAction(getCardForNonBuildAction(computer_player_id), BrassIndustryEnum.COAL.getValue(), BrassIndustryEnum.COAL.getValue());
						coalTech++;
						return computer_move;
					}
					else if(ironTech == 1)
					{
						computer_move.selectTechUpgradeAction(getCardForNonBuildAction(computer_player_id), BrassIndustryEnum.IRON.getValue(), BrassIndustryEnum.IRON.getValue());
						ironTech++;
						return computer_move;
	...
	private void firstTurn(BrassComputerPlayerAction computer_move, int player_id, int industry_id)
	{
